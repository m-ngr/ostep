...
_name_ of the file and the _mode_. The latter just indicates what we plan to
...
Second, note the _critical_ checking of whether the **fopen()** actually
...
more about _why_ the error occurred; learn about those on your own (using
...
- If _no files_ are specified on the command line, **wcat** should just exit
...
- If a search term, but no file, is specified, **wgrep** should work,
  ...
  _standard input_. Doing so is easy, because the file stream **stdin**
...
_run-length encoding_ (_RLE_). RLE is quite simple: when you encounter **n**
