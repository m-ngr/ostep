...
void make_test_file(bool should_succeed, char* desc, char* error, int offset,
                    void* new_bytes, size_t new_bytes_len);
...
int main(int argc, char** argv) {
  ...
  superblock sb =
      (superblock){xint(FSSIZE),   xint(NBLOCKS),    xint(NINODES),  xint(NLOG),
                   xint(LOGSTART), xint(INODESTART), xint(BMAPSTART)};

  // Now that our data is intialized, it's time to start writing to the disk
  // image file
  fsfd = open_test_file();
  if (fsfd < 0) {
    perror(argv[1]);
  ...
  dinode inodes[4] = {
      ...
      (dinode){
          xshort(T_DIR),  // type
          0,
          0,                 // major and minor (not relevant)
          xshort(1),         // nlink
  ...
  u8 bitmap[13] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
                   0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
  assert(DATASTART * BSIZE == write_bytes(bitmap, 13 * sizeof(u8)));

  // write the root directory's dirents
  dirent dirents[4] = {
      (dirent){xshort(1), {'.'}}, (dirent){xshort(1), {'.', '.'}},
      (dirent){xshort(2), {'h', 'e', 'x', '.', 't', 'x', 't'}},
      (dirent){xshort(3),
               {'l', 'e', 't', 't', 'e', 'r', 's', '.', 't', 'x', 't'}}};
  ...
  offset = INODESTART * BSIZE + 3 * sizeof(dinode) + offsetof(dinode, addrs) +
           3 * sizeof(u32);
  ...
  offset = INODESTART * BSIZE + 20 * sizeof(dinode) + offsetof(dinode, addrs) +
           3 * sizeof(u32);
  ...
  offset = INODESTART * BSIZE + 2 * sizeof(dinode) + offsetof(dinode, addrs) +
           NDIRECT * sizeof(u32);
  bad_inode_addr = xint(BMAPSTART);
  make_test_file(false, desc, error, offset, &bad_inode_addr, sizeof(u32));
  ...
  make_test_file(false, desc, error, offset, &bad_inode_name,
                 strlen(bad_inode_name));
  ...
  // 4th indirect address of letters.txt is 0x5D, which is bit 5 of byte 11 in
  // the bitmap
  ...
  // Address to indirect block of hex.txt is 0x48, which is bit 0 of byte 9 in
  // the bitmap
  offset = BMAPSTART * BSIZE + 9;
  bad_bitmap_byte = 0b01111111;
  make_test_file(false, desc, error, offset, &bad_bitmap_byte, sizeof(u8));
  desc = "4th indirect address of hex.txt is marked free\n";
  // 4th indirect address of hex.txt is 0x4C, which is bit 4 of byte 9 in the
  // bitmap
  offset = BMAPSTART * BSIZE + 9;
  ...
  desc =
      "Byte 14 of bitmap is 0b10000000, marking free address 104 (0x68) as in "
      "use \n";
  ...
  desc =
      "hex.txt is deleted from the root directory, but still exists as a used "
      "inode\n";
  ...
  desc =
      "letters.txt is moved from inode 3 to inode 4 free but still referenced "
      "as inode 3\n";
  offset = INODESTART * BSIZE + 3 * sizeof(dinode);
  dinode bad_inode_pair[2] = {{}, inodes[3]};
  make_test_file(false, desc, error, offset, &bad_inode_pair,
                 sizeof(bad_inode_pair));
...
int open_test_file() {
  char filename[16];
  assert(0 <
         snprintf(filename, sizeof(filename), "./tests/%d.img", test_counter));
  test_counter++;
...
void make_test_file(bool should_succeed, char* desc, char* error, int offset,
  ...
  assert(0 <
         snprintf(filename, sizeof(filename), "./tests/%d.desc", test_counter));
  make_file_with(filename, desc);

  // make the test #.run file
  assert(0 <
         snprintf(filename, sizeof(filename), "./tests/%d.run", test_counter));
  char run_data[32];
  ...
  assert(0 <
         snprintf(filename, sizeof(filename), "./tests/%d.rc", test_counter));
  if (should_succeed) {
  ...
  assert(0 <
         snprintf(filename, sizeof(filename), "./tests/%d.err", test_counter));
  make_file_with(filename, error);

  // make the test #.out file
  assert(0 <
         snprintf(filename, sizeof(filename), "./tests/%d.out", test_counter));
